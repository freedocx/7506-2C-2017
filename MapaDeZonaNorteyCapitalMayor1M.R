...
datatomap <- datatomap %>% subset(price_aprox_usd < 8600000)
...
ggmap(map) +
  geom_point(data=datatomap, aes(x=lon, y=lat, colour = price_aprox_usd ),
             size = (datatomap$price_aprox_usd*500)/aux + 0.5,
             alpha =0.35 + (datatomap$price_aprox_usd*100)/aux )  +
  scale_color_gradient(high="#F00000",low="#6363FF", name="Precio en Millones de Dólares  ") +
