...
#library(extrafont)
## + 700 ocurrencias
byName <- byName %>% subset(.,promedio<176)
...
toaxis  = subset(byName,byName$promedio< 176)
...
ggplot(byName, aes(variable, promedio, label = promedio),scale="free") +
  ...
  xlab("Palabras") +
  ylab("Promedio de precios en miles de Dólares") +
  ggtitle("Palabras con menor promedio de precios") + 
  ...
  theme(axis.text.x = element_blank(), 
        ...
        axis.text.y = element_text(hjust = 0, angle = 0, 
                                   ...
                                   margin = margin(t = 0, r = -125, b = 0, l = 20)),
