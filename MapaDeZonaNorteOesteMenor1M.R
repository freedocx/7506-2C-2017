...
map <- get_googlemap(center = c(-58.69,-34.55),zoom=11,maptype = "roadmap",
...
ggmap(map) +
  geom_point(data=datatomap, aes(x=lon, y=lat, colour = price_aprox_usd ),
             size = (datatomap$price_aprox_usd*6000)/aux + 0.5,
             alpha =0.3 + (datatomap$price_aprox_usd*100)/aux )  +
  scale_color_gradient(high="#F00000",low="#FED4FF", name="Precio en Miles de Dólares  ") +
